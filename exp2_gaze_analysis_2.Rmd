---
title: "Analysis_gaze_delay_period"
author: "Dongyu Gong"
date: "23 May 2024"
output:
...
```{r}
hist(et_dat$meanxp_baselined[abs(et_dat$meanxp_baselined) < (960-725)/2],100)
excl <- et_dat[abs(et_dat$meanxp_baselined) > (960-725)/2,]
...
```{r}
hist(et_dat$meanyp_baselined[abs(et_dat$meanyp_baselined) < (775-540)/2],100)
...
excl <- et_dat[abs(et_dat$meanyp_baselined) > (775-540)/2,]
...
# Item_dir_lr: X data
```{r}
#Let's have a new variable dealing with neutral cue
neutral_data <- read_excel("neutral_info_exp3.xlsx")
neutral_data <- mutate(neutral_data,item_lr = case_when(
        item_loc %in% c(1,3) ~ 'left',
        item_loc %in% c(2,4) ~ 'right'))
neutral_data$block <- neutral_data$block + 80
neutral_data.select <- neutral_data %>% dplyr::select(participant,block,item_lr,item_category)
neutral_data.select <- as.data.frame(neutral_data.select)
neutral_data.select$block <- as.integer(neutral_data.select$block)
xdat <- left_join(xdat,neutral_data.select,by = c("participant","block"))
xdat$item_dir_lr <- xdat$cue_dir_lr
xdat$item_dir_lr[!is.na(xdat$item_lr)] <- xdat$item_lr[!is.na(xdat$item_lr)]
```

#Item_dir_tb: Y data
```{r}
neutral_data <- mutate(neutral_data,item_tb = case_when(
        item_loc %in% c('1','2') ~ 'top',
        item_loc %in% c('3','4') ~ 'bottom'))
neutral_data.select <- neutral_data %>% dplyr::select(participant,block,item_tb,item_category)
neutral_data.select <- as.data.frame(neutral_data.select)
neutral_data.select$block <- as.integer(neutral_data.select$block)
ydat <- left_join(ydat,neutral_data.select,by = c("participant","block"))
ydat$item_dir_tb <- ydat$cue_dir_tb
ydat$item_dir_tb[!is.na(ydat$item_tb)] <- ydat$item_tb[!is.na(ydat$item_tb)]
```

# ADDED: aggregate for neutral cue probe period: X
```{r}

aggX <- data.table::setDT(xdat)[,lapply(.SD, mean),
                                by=.(participant, item_dir_lr, cue_category, item_category, cueFrame), 
...
# ADDED: aggregate for probe period: Y
```{r}
aggY <- data.table::setDT(ydat)[,lapply(.SD, mean),
                                by=.(participant, item_dir_tb, cue_category, item_category, cueFrame),
...
# ADDED: neutral condition: left and right: X
```{r}
aggX <- aggX[aggX$cue_category=='Neutral' & aggX$item_dir_lr!='neutral',]
table_lr<-summarySEwithin(data=aggX, measurevar = "meanxp_baselined", withinvars = c("item_dir_lr","item_category","cueFrame"),idvar = "participant",na.rm = TRUE)
table_lr$time <- as.numeric(table_lr$cueFrame)

table_lr$item_category <- factor(table_lr$item_category, levels=c('WM', 'LTM'))
table_lr$item_dir_lr <- factor(table_lr$item_dir_lr, levels=c('left', 'right'))

ggplot(table_lr, aes(x = time, y = meanxp_baselined)) + 
    labs(y = "Offset (pixels)", x = "Time relative to probe onset (ms)", color="Neutral cue, probe direction") + 
    geom_hline(yintercept=0, linetype="dashed",size=1, alpha=1) +
    geom_line(aes(color = item_dir_lr),alpha = 1,size=1) +
    geom_ribbon(aes(ymax=meanxp_baselined+se, ymin=meanxp_baselined-se, fill=item_dir_lr), alpha = 0.2) +
...
ggsave("figures/neutral_bias_RL.svg", device = "svg",width = 25, height = 15, units = "cm")
```
...
ggplot(table_lr, aes(x = time, y = meanxp_baselined)) + 
    labs(y = "Offset (pixels)", x = "Time relative to cue onset (ms)", color="Cue direction") + 
    geom_hline(yintercept=0, linetype="dashed",size=1, alpha=1) +
    ...
    theme_set(theme_gray(base_size = 20)) +
    scale_color_manual(labels=c("neutral","left","right"),
                       values = c("gray","blue", "red")) + 
    scale_fill_manual(labels=c("neutral","left","right"),
                      values = c("gray","blue", "red")) +
    # scale_y_continuous(breaks = seq(-4, 2, by = 2)) +
    guides(fill="none") +
    theme(plot.title = element_text(hjust = 0.5),         
...
```
# ADDED: neutral condition: top and bottom: Y
```{r}
aggY <- aggY[aggY$cue_category=='Neutral' & aggY$item_dir_tb!='neutral',]
table_tb<-summarySEwithin(data=aggY, measurevar = "meanyp_baselined", withinvars = c("item_dir_tb","item_category","cueFrame"),idvar = "participant")
table_tb$time <- as.numeric(table_tb$cueFrame)

table_tb$item_category <- factor(table_tb$item_category, levels=c('WM', 'LTM'))
table_tb$item_dir_tb <- factor(table_tb$item_dir_tb, levels=c('bottom', 'top'))

ggplot(table_tb, aes(x = time, y = -meanyp_baselined)) + 
    labs(y = "Offset (pixels)", x = "Time relative to probe onset (ms)", color="Neutral cue, probe direction") + 
    geom_hline(yintercept=0, linetype="dashed",size=1, alpha=1) +
    geom_line(aes(color = item_dir_tb),alpha = 1,size=1) +
    geom_ribbon(aes(ymax=-meanyp_baselined+se, ymin=-meanyp_baselined-se, fill=item_dir_tb), alpha = 0.2)+
    theme_set(theme_gray(base_size = 20)) +
...
ggsave("figures/neutral_bias_TB.svg", device = "svg", width = 25, height = 15, units = "cm")
```
...
```{r}
aggY <- aggY[!is.na(aggY$meanyp_baselined),]
...
ggplot(table_tb, aes(x = time, y = -meanyp_baselined)) + 
    ...
    theme_set(theme_gray(base_size = 20)) +
    scale_color_manual(labels=c("neutral","bottom","top"),
                       values = c("gray","blue", "red")) + 
    scale_fill_manual(labels=c("neutral","bottom","top"),
                      values = c("gray","blue", "red")) + 
    # scale_y_continuous(breaks = c(-4,-2,0,2),limits = c(-4,3)) +
    guides(fill="none") +
    theme(plot.title = element_text(hjust = 0.5),         
...
toward <- dplyr::mutate(toward, meanxp_baselined.y=zoo::na.approx(meanxp_baselined.y, na.rm = FALSE, rule = 2))
...
(towardnessX <- ggplot(table_lr_t, aes(x = time, y = meanxp, fill=cue_category)) + 
...
```
# cluster-based permutation test- X data - DEBUG
```{r}
#LTM
...
zeros <- eyeX
zeros[3:ncol(eyeX)] <- 0
zeros$cue_category <- "zero"
eyeX <- rbind(eyeX,zeros)
...
#Draw the figure
(towardness_sigX <-towardnessX + annotate("point", x = sig_LTM_WM , y =-0.6, colour = "black", size=1) + 
    annotate("point", x = sig_WM , y =-0.5, colour = rgb(65/255,143/255,111/255),size=1)+
    annotate("point", x = sig_LTM , y =-0.4, colour = rgb(138/255,118/255,190/255),size=1))
...
(towardness_sigY <-towardnessY + annotate("point", x = sig_LTM_WM , y =-0.6, colour = "black", size=1) + 
    annotate("point", x = sig_WM , y =-0.5, colour = rgb(65/255,143/255,111/255),size=1)+
    annotate("point", x = sig_LTM , y =-0.4, colour = rgb(138/255,118/255,190/255),size=1))
    # scale_y_continuous(breaks = c(0,1,2),limits = c(-1,2))
